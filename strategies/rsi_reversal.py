...
Long  signal: RSI crosses back above the oversold threshold
Short signal: RSI crosses back below the overbought threshold
...
from strategies.indicators import compute_rsi
...
def _signals(candles: list, params: dict) -> list[dict]:
    ...
    for i in range(1, len(candles)):
        if rsi[i] is None or rsi[i-1] is None:
            continue
        if direction in ("long", "both") and rsi[i-1] <= oversold < rsi[i]:
            signals.append({"candle_idx": i, "direction": "long"})
        if direction in ("short", "both") and rsi[i-1] >= overbought > rsi[i]:
...
def indicators(candles: list, params: dict) -> dict:
    period = int(params.get("period", DEFAULTS["period"]))
    return {"rsi": compute_rsi(candles, period)}
