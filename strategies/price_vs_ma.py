"""
strategies/price_vs_ma.py
--------------------------
Price vs moving average strategy.

Long  signal: close crosses above the MA
Short signal: close crosses below the MA
...
def _signals(candles: list, params: dict) -> list[dict]:
    ...
    ma      = compute_ma(candles, period, ma_type)
    ...
    for i in range(1, len(candles)):
        ...
        if direction in ("long", "both") \
                and closes[i-1] <= ma[i-1] and closes[i] > ma[i]:  # type: ignore[operator]
...
def indicators(candles: list, params: dict) -> dict:
    period  = int(params.get("period",  DEFAULTS["period"]))
    ma_type = params.get("ma_type",     DEFAULTS["ma_type"])
    return {"fast_ma": compute_ma(candles, period, ma_type)}
